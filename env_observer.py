...
async def main() -> None:
    ...
    try:
        ...
        await data_writer.backup(f"testdata/board_data_backup_light_100.sqlite3")
        ...
        for level in [20, 40, 50, 60, 80]:
            ...
            await data_writer.backup(f"testdata/board_data_backup_light_{level}.sqlite3")
            await data_writer.clearAll()
        ...
        for level in [0, 25, 50, 75, 100]:
            ...
            current_temp = await get_average_temperature()
            await data_writer.backup(f"testdata/board_data_backup_fan_{level}.sqlite3")
            await data_writer.clearAll()
